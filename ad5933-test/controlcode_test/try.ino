...
void calibrateAD5933() {
  ...
  for (int i = 0; i < NUM_INCR + 1; i++, cfreq += FREQ_INCR / 1000) {
    // Compute impedance
    double magnitude = sqrt(pow(gain[i], 2) + pow(phase[i], 2));
    gain[i] = 1 / (magnitude * gain[i]);
  }

  // Calculate and store the high impedance check value
  double magnitude = sqrt(pow(gain[0], 2) + pow(phase[0], 2));
  double impedance = 1 / (magnitude * gain[0]);
  impedanceCheck = (int)impedance;
}
...
void setup(void) {
  ...
  // Call the calibration function once during setup
  calibrateAD5933();
}
...
bool frequencySweepEasy(int pin) {
  ...
  if (AD5933::frequencySweep(real, imag, NUM_INCR + 1)) {
    ...
    for (int i = 0; i < NUM_INCR + 1; i++, cfreq += FREQ_INCR / 1000) {
      ...
      // Use the stored calibrated values to compute impedance
